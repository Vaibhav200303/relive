...
- Save-to-device for the current viewer media and contextual multi-select download from Media or a Moment media gallery, with cancellable determinate progress and completion feedback (ADR-0099, ADR-0100).
...
- System/Light/Dark appearance plus the icon-matched **Ivory Gold fresh-install default**, Warm Journal, Ivory Gold, and the retained premium token sets. Each global palette synchronizes one matching launcher icon shared by its Light and Dark modes; Android applies alias changes immediately with a retained safe-boundary retry and iOS keeps Velvet Rose as an alternate to the Ivory Gold primary. Custom timelines persist a dedicated `TimelineAppearance` independently of global app appearance; themes otherwise change only presentation.
...
- Gate scheduled automatic backup, creation beyond three custom timelines, premium appearance, and PDF/portable-archive creation; retain manual backup, every restore operation, free opening of `.relive`, existing archive access, Warm Journal/Ivory Gold/Velvet Rose palettes, and Warm Cream/Blush Pink wallpapers in Free.
...
- **Approved phase override — first-launch onboarding:** add the five-chapter, skippable, versioned Capture → Organize → Relive → Privacy → Ready introduction from PRODUCT_SPEC §1A, ADR-0092, and ADR-0093. It is shared Compose UI with a persistent native-component scene, project-owned sample photography, five existing dark app palettes joined by an interpolated atmospheric gradient, native install preferences, existing-user protection through bounded Profile counts, no permission request, no archive hydration, and no new runtime dependency. Optional-backup copy is presentation-only; no sync backend is included. This slice is implemented now at explicit product-owner request while earlier in-progress phases retain their status.
