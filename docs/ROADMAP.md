...
- **Approved phase override — first-launch onboarding:** add the reference-matched welcome screen plus five-page, skippable, versioned introduction from PRODUCT_SPEC §1A, ADR-0073, ADR-0084, and ADR-0085. It is shared Compose UI with original project-owned raster heroes, native install preferences, existing-user protection through bounded Profile counts, no permission request, no archive hydration, and no new runtime dependency. The optional cloud-backup copy is presentation-only; no sync backend is included. This slice is implemented now at explicit product-owner request while earlier in-progress phases retain their status.
