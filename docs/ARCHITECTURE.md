...
## 16. Diary PDF and portable archive export

Shared export use cases build immutable `MagazineDocument` and `PortableArchiveSnapshot` values from repository reads. PDF rendering, ZIP I/O, native document/photo pickers, save/share sheets, and incoming document-open requests stay behind platform boundaries. The shared diary-document builder emits escaped offline HTML/CSS, inline vector doodles, and only local image references; platform renderers prohibit network loading. Both renderers bind the same bundled Kalam handwriting files so diary text remains consistent and searchable across Android and iOS.

`.relive` v1 is a standard ZIP STORE container with MIME type `application/vnd.vaibhav.relive.archive`. `manifest.json` identifies and hashes the format and every media entry, `archive.json` holds the portable domain graph, and `media/<sha256>.<ext>` deduplicates source media and timeline covers. Readers validate identity/version, safe paths, declared size/hash, duplicate identities, relationships, and inventory before exposing a session. A session-local `MediaStore` is read-only and materializes media lazily; no portable viewer adapter can reach live repositories. This format is intentionally unrelated to the Google Drive restore package.
