...
- **Saved readable location** — displayed below the date chip with surrounding whitespace trimmed and only its first character capitalized; persisted data is unchanged.
...
- A fresh installation with onboarding version `0` and bounded Profile counts of zero shows exactly five chapters in order: Capture, Organize, Relive, Privacy, Ready. It performs no whole-archive read. With no saved native appearance preference, the app and launcher start in Ivory Gold.
...
- UI tests cover all five fixed headings/chapters, progress semantics, Skip visibility, both final actions, 48dp targets, compact heights, large font scaling, rapid input, and reverse navigation. Runtime verification covers all four forward/reverse shared transitions, continuous background blending, and the bounded Relive preview. The chapters use the existing dark Evergreen → Original → Plum & Gold → Ivory Gold → Velvet Rose palettes; background canvas, glow, tint, accent, and shadow interpolate from the same transition progress, and system bars retain light icons. Reduced motion uses short fades or immediate stable spatial state and runs no travel, scale, morph, stagger, tilt, or idle loop.
- Launcher tests cover palette-to-icon mapping, Android's queued atomic on-stop swap with a safe pending retry, alias resolution for suffixed build variants, Ivory Gold as the manifest baseline, and Velvet Rose as the iOS alternate; notification tests verify the theme-neutral monochrome Relive diary mark. On a device, selecting a palette must leave the current activity resumed and its process alive; launcher search must show exactly one Relive entry before and after pressing Home, with the selected icon afterward. Reopening from the changed alias and pressing Back must never reveal a second activity with the prior theme; launcher, widget, share, and archive entries must reuse the single `MainActivity` task. Verify every placed Quick Capture widget redraws from its updated Glance state for palette and light/dark changes without opening the widget or retaining an earlier session palette, including after process recreation. Legacy `sunrise` and `sunset` preferences decode to Ivory Gold and Velvet Rose, while new writes use `ivory_gold` and `velvet_rose`.
...
- [ ] Android share of text, URL, image, video, audio, and a supported mixed batch opens Relive's timeline picker, which looks and scrolls exactly like the Timelines screen — same canvas, search field, `YOUR TIMELINE` heading, and cards in the same order — headed `Choose a timeline`, with no profile affordance, no `+`, no floating navigation, and no long-press rename/delete. Its search field narrows the list the same way.
- [ ] Selecting a timeline morphs that card into its detail screen exactly as tapping it on the Timelines screen does, then the existing inline composer expands with the shared media already attached as ordered processing placeholders; Keep Moment saves it.
- [ ] A timeline tapped while the share is still being read stays lit and opens as soon as the payload lands, rather than swallowing the tap. With no custom timelines, the picker shows its empty placeholder and Back is the only way out.
...
## 14. Export and portable archive

- Common tests cover inclusive scope/date filtering for both formats, oldest-first output, date-page grouping, coalesced missing-date interludes, centered single-Moment treatment, image-only PDF media, Standard/HD image-quality propagation, cover defaults and cleanup, stable filename sanitization, HTML escaping, frozen portable timeline identity, Pro creation gates, progress/cancellation, leave-running Back behavior, hidden completion notification exactly-once behavior, Profile export status copy, manifest/snapshot round trips, deterministic STORE ZIP fixtures, SHA-256, corrupt data, unsafe paths, duplicate identities, relationship validation, and read-only presentation.
- UI tests cover Profile → Export, the separate Profile → Open Relive archive action, Upgrade return, free archive opening, format-first PDF/archive setup, one-off cover cleanup, empty/missing-media errors, full-screen progress/result/error states, visible and system Back behavior at each stage, Save/Share completion actions, and the absence of mutation controls in portable mode.
- Android tests cover picker/CreateDocument/FileProvider/ACTION_VIEW routing plus Standard image bounds/encoding/temp cleanup and notification permission gating; iOS tests cover document UTI/open-URL/export/share routing plus matching Standard/HD image preparation and authorized-only immediate notifications. Cold/warm starts must preserve App Lock as the outer gate and must bypass onboarding without completing it.
- Fixture PDFs cover text-only and long-form Moments, one and multiple Moments per date, a short image-and-feeling Moment before long copy on the same date, missing-date spans across month/year boundaries, Unicode, large and multiple images, tags, readable locations, feelings, long cover strings, every paper palette, and Moments containing audio/video to confirm those attachments are absent. Render every page with Poppler and inspect clipping, overlap, emoji glyphs, taped-photo treatment, complete uncropped images, count-specific multi-image layouts, date boundaries, gap pages, and unlabelled date-free photo pages. Confirm that no photo or frame crosses an A4 boundary, that the selected paper reaches all four edges without white fallback on every continuation sheet, and that every sheet has the coordinated double-line frame plus safe top/bottom content clearance; then verify adaptive readable ink, large direct-on-paper handwriting hierarchy, lighter non-obscuring edge doodles, A4 page boxes, searchable text, bundled Kalam rendering with no network dependency, and Android/iOS visual consistency.
