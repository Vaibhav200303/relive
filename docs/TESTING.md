...
- Common tests cover inclusive scope/date filtering for both formats, oldest-first output, date-page grouping, coalesced missing-date interludes, centered single-Moment treatment, image-only PDF media, cover defaults, stable filename sanitization, HTML escaping, frozen portable timeline identity, Pro creation gates, progress/cancellation, manifest/snapshot round trips, deterministic STORE ZIP fixtures, SHA-256, corrupt data, unsafe paths, duplicate identities, relationship validation, and read-only presentation.
...
- Android tests cover picker/CreateDocument/FileProvider/ACTION_VIEW routing; iOS tests cover document UTI/open-URL/export/share routing. Cold/warm starts must preserve App Lock as the outer gate and must bypass onboarding without completing it.
