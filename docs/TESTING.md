...
- Fixture PDFs cover text-only and long-form Moments, one and multiple Moments per date, missing-date spans across month/year boundaries, Unicode, large and multiple images, tags, readable locations, feelings, long cover strings, and Moments containing audio/video to confirm those attachments are absent. Render every page with Poppler and inspect clipping, overlap, glyphs, taped-photo treatment, complete uncropped images, count-specific multi-image layouts, date boundaries, gap pages, and unlabelled date-free photo pages. Confirm that no photo or frame crosses an A4 boundary, then verify direct-on-paper dark handwriting, lighter non-obscuring doodles, A4 page boxes, searchable text, bundled Kalam rendering with no network dependency, and Android/iOS visual consistency.
