...
Android accepts a user-initiated system share of plain text/URLs, images, videos, audio, or a supported mixed batch. Relive first asks which timeline should receive one new Moment, on a surface that is the Timelines screen itself with every control that does not choose a destination removed: the custom timelines in their normal order under a `Choose a timeline` heading, with Back returning to the app the content was shared from. The shared content is not previewed there — choosing a timeline opens that timeline directly, with the share already merged into its inline composer, where the person adds a title and content before Keep Moment saves it, creating the normal All + custom membership. A share cannot target All: All is not a card on the Timelines screen, so it is not one here either. Shared content only pre-fills the existing inline composer and is never saved without the user choosing Keep Moment. Unsupported, unreadable, or mixed batches containing unsupported files are rejected as one request; arbitrary documents are not attachments in v1. This capability is Android-only for now.
...
- Entry from global `+ New` brings Home into focused All moments and runs this same in-place expansion transition on the existing rail composer, **travelling** to seat the composer and its Keep Moment button rather than arriving at them: the surface covers the distance at the same steady pace the return-to-newest control moves at, in whichever direction the composer lies, and lands exactly on it. This is the same movement whether the composer is closed or **already open** — pressed from deep in the feed with an open composer above, `+ New` is a request to go back to it, so it carries the person there instead of doing nothing, and it re-prepares nothing, keeping the date, timeline assignments and anything else already set. The rail `+` opens the composer the same way. There is no navigation, no waiting on a fresh content projection, no settled-frame handoff, and no arbitrary delay. Per ADR-0059 the composer requests no field focus and opens no IME — the person taps a field when ready — and subsequent media actions do not request focus either.
...
- Empty queries show `Find anything you've saved.`, compact suggestion chips, and up to five process-session recent searches. Below ten saved Moments, the chips use the fixed vocabulary (`beach`, `birthday`, `college`, `friends`, `food`, `notes`). At ten or more Moments, they instead derive from the archive's most-used saved tag and readable-place labels, retaining the matching Tags or Places mode; fixed vocabulary remains the fallback when no dynamic terms exist. Choosing a suggestion or recent item runs it; recent items can be removed individually or cleared together. Empty queries never render the whole archive. No results show `No moments found.`
...
Profile is an auxiliary destination opened from the Home surface's profile affordance; it is not a bottom-navigation destination. It shows the profile identity — a real display name when one is set, otherwise a neutral identity placeholder — and this is the only source for Home's `Hey, {name}!!!` greeting; with no real display name Home shows exactly `Hey!!!` (§2). It also shows installation joining date when known, informational Moment/custom-Timeline/place counts, an inline Appearance card, and the approved remaining Profile IA: Preferences; Media & storage; Backup; Export; Open Relive archive; Location; Reminders; Privacy & security; Help & feedback; About Relive. Appearance provides persistent System/Light/Dark and app-default palette controls. Media & Storage is a read-only archive-insights screen showing Relive-managed attachment storage and category counts; it provides no optimization, deletion, cleanup, or device-wide storage controls. Help & feedback is a task-oriented support surface with expandable practical answers, a release-configured support address, and feedback/problem-report mail actions that expose only app/build and platform diagnostics. About Relive is the product story and full feature guide; it owns version, Privacy Policy, Terms of Service, and open-source licenses. Other Profile-row functionality remains deferred unless a real destination exists.
...
Future Settings entries remain Themes and Upgrade to Pro. Export is defined below and by ADR-0075.

### Export

Profile → Export offers the two Pro-only creation formats. The separate free **Open Relive archive** Profile row launches the native document picker; compatible files opened from Android Files or another provider route to the same read-only viewer. A Keepsake PDF is an A4, oldest-first diary of All moments or one custom timeline, optionally filtered by an inclusive device-local date range. Its cover title/subtitle are editable, its optional one-off cover photo is temporary export input, and its per-export diary paper may be Warm cream, Blush pink, Sage green, Lavender, Powder blue, or Soft peach; none of these choices become archive data. Every paper maps automatically to a readable dark ink, accent, ruling, and lighter doodle color. That paper fills every physical sheet, including continuation space below short content, and every sheet carries a visible coordinated double-line keepsake border around a safe content inset. Every observed date begins on a fresh page; its date and complete Moment writing use large bundled handwriting directly on ruled paper, while varied edge-weighted organic and memory doodles fill unused space without reducing legibility. A felt Moment prints its feeling as one large recognizable emoji after its writing and tags, never as the enum word. One short Moment is given a centered page, multiple text-only Moments on a date may share paper, but a media-bearing Moment and all of its following photo sheets finish before the next Moment begins. Long writing is balanced across its required continuation sheets, with a smaller first-page allowance for the date and Moment heading and a larger continuation-page allowance that uses the available printable area without crowding; the date is printed only on the first page and continuation pages carry no date or “continued” label. A short image-bearing Moment keeps its first photograph composition with its timestamp, copy, and feeling even when its date also contains other Moments, so it never reads as an emoji-only entry. A missing run between observed dates becomes one reflective no-Moments interlude. Images remain readable pasted photographs: the complete image is visible inside its frame, one is generous, two to four use count-specific scrapbook arrangements, and every overflow sheet uses a fuller complete A4 composition so no photo grid can cross a page boundary. The PDF prints complete text and image attachments only; audio and video are omitted. Empty selections and selections with unreadable referenced images cannot be exported.

A `.relive` file is a versioned, unencrypted, view-only copy of the selected timeline and inclusive date range. It freezes that timeline's name, wallpaper, Moment theme, and custom cover photo at export time, so another person opens the intended timeline identity even if the live source is edited later. Its text and source media remain playable in a temporary read-only viewer. It never imports, merges, restores, or writes to the live database. App Lock remains the outer gate; authoritative external opens bypass onboarding without completing it. Session media is materialized only when viewed and session files are removed on close or next launch. Google Drive Backup & Restore and its wire format remain independent and unchanged.

Both formats disclose that exported files can contain private text, media, and readable location information. PDF personalization offers Standard image quality (maximum 1280 px long edge, JPEG quality 70) for a smaller file suited to sharing and HD image quality (maximum 1920 px long edge, JPEG quality 82) for sharper printing; Standard is the default. Creation requires an active Relive Pro entitlement when generation begins; an authorized generation may finish if entitlement state changes. Leaving the Export screen while creation is processing returns to Profile without cancelling the generation; the screen restores its processing/result/error stage when reopened, and an explicit Cancel export action remains available. Profile's Export row reflects hidden or visible work with `Creating PDF` (and progress when available), `Export ready`, or `Export needs attention`. When Export is hidden, a terminal outcome may post one generic completion or failure notification only if notification permission already exists; export never requests that permission and never puts a timeline name, filename, Moment content, media, or location in notification copy. Opening `.relive` remains free forever.
