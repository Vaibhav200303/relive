...
Both formats disclose that exported files can contain private text, media, and readable location information. Creation requires an active Relive Pro entitlement when generation begins; an authorized generation may finish if entitlement state changes. Opening `.relive` remains free forever.
