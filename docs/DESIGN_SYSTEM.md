...
Moving between Home's two states — the welcome/Rediscover top state and focused All moments — is a **reversible in-surface scroll-state change**: the motion is the scroll itself. It uses no fade-through, no container transform, and no shared-axis pattern, because no navigation event, route change, or back-stack entry occurs. A read-only collection route opened from a Rediscover card is an **M3 Expressive container transform** (ADR-0065): the card morphs into the screen over `long2` on the emphasized curve, exactly as a custom timeline's card does (ADR-0063), because the card's cover is retained as the screen's cover; Back runs it in reverse, and reduced motion falls back to the fade-through, which also still governs the Home ↔ All-theme route.
