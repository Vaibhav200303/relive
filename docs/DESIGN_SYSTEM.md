...
The Keepsake uses one fixed A4 portrait diary system: warm cream ruled paper, dark handwritten Kalam ink, Fraunces/Inter supporting type, a full cover, restrained metadata, and varied low-contrast doodles. Its print hierarchy prioritizes comfortable viewing on a phone as well as paper: Moment writing is `15pt`, secondary metadata is at least `10.5pt`, and compact tags are at least `8.5pt`. Dates, titles, and Moment text are written directly on the page rather than placed inside cards. Every observed date starts with a large ordinal date at the top of a fresh page. A single short Moment rests vertically centered beneath it; multiple Moments from that date flow on the paper and may continue onto additional pages without allowing the next date to share a page. Only the first page names the date; continuation pages are intentionally unlabelled and never say “continued.” Missing dates are never expanded one by one: each uninterrupted gap between observed dates becomes one full-page pause naming the missing span, followed by “No moments were kept here. Life was still being lived.” Text is never truncated. Image attachments appear as lightly rotated, white-bordered photographs with translucent tape and shadow; each frame contains the complete uncropped image, one photo is generous, and two to four photos use count-specific scrapbook compositions. Overflow compositions are isolated as complete A4 photo pages so neither a photograph nor its frame can straddle a page boundary. Doodles use a deliberately lighter sepia than written content and vary between pages with botanical sprigs, hearts, stars, paperclips, cameras, suns, rainbows, cups, houses, and curved arrows. Audio and video are omitted completely. Raw coordinates and internal identities are never printed.
