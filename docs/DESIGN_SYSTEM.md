...
The Rediscover row reuses Warm Journal typography, colors, radii, borders, and spacing. Its cards are `Favourites`, `On This Day`, `From Your Past`, and `All Photos`, sharing a single card family; none of them is an entry point into the All moments timeline, which is already on this surface. The row is a Material 3 multi-browse carousel: the focal card renders at full size while trailing cards compress into masked items that read as imagery. Each card is one full-bleed cover (ADR-0064) — one shuffle-chosen visual attachment when the collection has preview media, otherwise a generated accent-derived gradient (§19.1) — with a one-line ellipsized serif title overlaid at the lower edge of the focal card only, over a scrim that dims the cover exactly while that label is shown; cards carry no count, date, or other metadata line. Cards accept taps in every backdrop state: Home's backdrop-window item, which owns the hit test over the row, forwards taps to the card under them the same way it proxies horizontal drags into the carousel. `All Photos` is a bounded, read-only system collection of Moments carrying at least one image or video attachment, read through the same bounded projection pattern as Favourites and From Your Past; its card cover is one shuffle-chosen candidate from that projection (§19.1).
