...
The approved UI reference is authoritative for the **All moments timeline band** — its rail, Moment cards, metrics, and tokens — and for custom timeline UI, except for the saved-Moment marker and metadata override in ADR-0101:
...
- Theme-gradient timeline rail, soft blurred circular markers, integrated plus-circle for composing.
...
| `type.eyebrow`          | sans   | 11 / 16, Medium, uppercase, wide tracking  | timeline location metadata        |
...
The composer location input uses `type.body` with muted/secondary text, the `icon.sm` location pin, and a `48dp` minimum row target. It is an inline editorial field directly below the composer's date and time, never a heavy outlined address form. A saved Moment displays an abbreviated `MMM d, yyyy` date in a light translucent highlight-accent pill after an explicit gap from its calendar marker. Marker, date, and favorite share one fixed 48dp centreline. Location uses a separate row beneath that top row, separated by `space.2`, in `type.eyebrow` and `color.text.secondary`, so its presence never shifts the top-row geometry. Presentation trims location whitespace and capitalizes only the first character. The saved time moves into the print's bottom-right in a low-emphasis `type.tag` treatment.
...
| Token                 | Value   | Usage                                     |
| --------------------- | ------- | ----------------------------------------- |
| `timeline.rail.width` | `4dp`   | theme-gradient vertical rail              |
| `timeline.marker.size` | `32dp` | soft-blur circle centred in the 48dp marker slot |
...
| `timeline.marker.color` | `color.accent` fading to transparent | soft blurred saved-Moment marker |

The rail uses a vertical primary → highlight accent → primary gradient derived from the active global theme, with restrained opacity so it remains behind the content hierarchy. The plus-circle is integrated into the rail (bordered, canvas fill). Because the All moments feed on Home is newest-first, the composer sits at the chronological end of the feed, which renders at its **head**, directly beneath the `All moments` heading; the rail terminates at the centre of the plus marker and no rail continues above it. Keeping a Moment collapses the composer back to this `+` marker and renders the saved Moment with a soft blurred marker adjacent to its date chip.
...
**Exception — Home Rediscover row (ADR-0064):** the Home surface's Rediscover collection cards have no lower information surface. Their cover — one visual attachment or a generated gradient — fills the whole card (`rediscover.compactCardHeight`), and the one-line title is overlaid at the card's lower edge over a black vertical legibility scrim that fades in only while the carousel's focal card shows its label; masked cards show pure, undimmed imagery. Every other collection-card family keeps the direct media/surface boundary above.
...
Onboarding is one persistent scene across five chapters. Its background colors and broad gradient positions interpolate rather than swap; the recognizable memory card interpolates position, size, and treatment across Capture → Organize → Relive → Privacy → Ready. The main scene uses `long3` (550ms) with emphasized easing. Supporting groups reveal through roughly 8–12dp travel, 0.98→1 scale, and overlapping progress thresholds equivalent to 40–70ms spacing. Copy crossfades independently so readable headings never overlap. Rapid target changes cancel and continue from the current scene progress. Reduced motion snaps spatial properties to the stable target and retains only short copy/action fades.

Moving between Home's two states — the welcome/Rediscover top state and focused All moments — is a **reversible in-surface scroll-state change**: the motion is the scroll itself. It uses no fade-through, no container transform, and no shared-axis pattern, because no navigation event, route change, or back-stack entry occurs. A read-only collection route opened from a Rediscover card uses the same direct full-screen fade character as the Mood insights overlay (ADR-0095): the destination fades over the departing Home surface and Back mirrors the fade, without shared card geometry. The card's cover is still retained as the collection screen's cover. Reduced motion uses the shared shorter fade. Home ↔ All-theme continues to use the same route fade.
...
Android external-share capture uses the same restrained system, and its picker **is** the Timelines surface (ADR-0069): the same canvas and grain, the same search field, the same `YOUR TIMELINE` heading, and the same custom timeline cards in the same order. Only two things differ — the wordmark is replaced by the `Choose a timeline` heading, and the profile affordance's slot holds Back to the source app. It carries no profile affordance, no creation control, no floating navigation, and no long-press rename/delete selection, so a card there has exactly one gesture; it also carries no preview of the shared content, which is reviewed in the composer it opens. It lists custom timelines only, so a share can no longer target All from the picker. The list is on screen from the first frame: the standard Timeline Home skeleton stands in until the summaries load, an error fades in over the list body, and a card chosen while the payload is still being read stays lit until the choice commits. Choosing a timeline is the **M3 Expressive container transform** of ADR-0063 run from the picker's card: `long2` on the emphasized curve, with the route swap cross-fading over the morph exactly as the Timelines swap does, after which the timeline settles collapsed for one frame before the normal composer expansion begins. Cancelling, the picker's own arrival, and reduced motion keep the short emphasized horizontal slide/fade. Material press/progress/error behavior is used under Relive tokens, without a global expressive theme.
...
The generated-cover fallback is a centralized mode-aware theme token. It derives curated rich gradient pairs from the active mode's workhorse accent — the same colour the `+ New` button wears — deepened toward black in light mode and lifted toward white in dark mode (ADR-0064), remains deterministic for a stable identity, and never modifies stored media. Global app appearance remains app-scoped. The editable All moments feed and every custom Timeline separately own a `TimelineAppearance`; staged rendering of its wallpaper and Moment treatment remains confined to timeline-owned regions — on Home that is the All moments band only, so the welcome block, the `Relive your memories` heading, and the Rediscover row always render on the plain canvas.
...
Profile archive-insights follows its approved visual reference (ADR-0082): an editorial storage hero with a full usage ring, a rounded Storage breakdown group containing category rows and proportional indicators, a rounded Your archive group containing category counters, and a quiet local-privacy card. The presentation may use nested tonal surfaces on this destination only. Its category distinctions derive only from semantic Relive colors and are always accompanied by labels and formatted values.
...
Collection-card visual regions use `ReliveGeneratedCover` when their preview data contains no eligible visual. The cover uses a deterministic stable hash of the Timeline ID (or logical `timeline-all`) or Moment ID to choose a curated gradient; outside the Home Rediscover row it is never persisted, time-based, or random. Each Home Rediscover card instead selects one image from its bounded projection using an epoch-hour bucket (ADR-0097): the choice holds still within the hour and advances at the boundary. Video and audio attachments are not eligible for this hourly photo cover. A generated cover remains the fallback when no image is available. Normal Timeline MomentCard media presentation remains unchanged.

The automatic bucketed collage cover is used by the All timeline's cover hero: its available visual candidates remain a bounded reactive projection, while a deterministic three-hour bucket chooses 1–9 distinct candidates and a curated layout. When there are zero candidates, it uses the neutral no-cover placeholder rather than a generated cover. The external-share timeline picker no longer carries an All destination card (ADR-0069). The `All Photos` card in Home's Rediscover row instead fills with one hourly-selected image from the same bounded projection (ADR-0097). The `All moments` band on Home carries no cover hero; it begins at its section heading and is not entered from a card.
...
Home opens with the welcome block: `Hey, {name}!!!` when a real profile display name exists and exactly `Hey!!!` when none does, with no placeholder or `Your Relive` fallback label, set in the serif brand role (`type.display` / `type.title`), followed by the subtitle `Your memories are waiting for you.` in `type.subtitle`. Home's backdrop expansion is a direct, reversible normalized-progress mapping, never an independent greeting tween: `Hey,` remains left-aligned and scales continuously 1.0×→1.32×; the inline name fades over 0–10%, while its fixed-coordinate second-line name fades over 80–100%. Endpoint text bounds are measured outside drag frames; both name layers are one-line only and long names use name-only scale-down before ellipsis. The subtitle's placement comes only from the interpolated greeting geometry. At 25–80%, the full-size Mood bar fades in and translates upward by `space.2`; its measured height plus `space.4 + space.4` is progressively allocated below the subtitle to move the Rediscover heading and row, with no width/height scaling or collapsed empty slot. `Relive your memories` and `All moments` are section headings in `type.eyebrow`, separated by `space.6` / `space.8`.

The Rediscover row reuses Warm Journal typography, colors, radii, borders, and spacing. Its cards are `Favourites`, `On This Day`, `From Your Past`, and `All Photos`, sharing a single card family; none of them is an entry point into the All moments timeline, which is already on this surface. The row is a Material 3 multi-browse carousel: the focal card renders at full size while trailing cards compress into masked items that read as imagery. Each card is one full-bleed cover (ADR-0064) — one hourly-selected image when the collection's bounded preview has photos, otherwise a generated accent-derived gradient (§19.1) — with a one-line ellipsized serif title overlaid at the lower edge of the focal card only, over a scrim that dims the cover exactly while that label is shown; cards carry no count, date, or other metadata line. Cards accept taps in every backdrop state: Home's backdrop-window item, which owns the hit test over the row, forwards taps to the card under them the same way it proxies horizontal drags into the carousel. `All Photos` keeps that sliding cover on entry, then presents every eligible image/video attachment as square tiles in the multi-media gallery's adaptive grid language. It has no Moment cards, rail, writing, tags, or metadata; tile taps open the full-screen viewer across the flattened visual collection (ADR-0098).

A tapped card opens its read-only collection as a **sliding-cover surface** (ADR-0065) running the custom-timeline mechanism of ADR-0062: the cover the card was wearing — the same resolved attachment or generated gradient — is the backdrop, titled on the cover the way a custom timeline's hero is, with the newest-first feed riding over it as a sheet and no app-bar heading anywhere. The surface opens at its top on the cover with no entry scroll. Its pinned slot carries **Back alone**: read-only collections have no timeline theme, no calendar, and no cover editing. Search is a top-level surface alongside Home: its Material 3 search field, counter, and up/down controls use Relive theme colors, typography, minimum touch targets, and calm active treatment; results retain Timeline presentation. Components must not introduce raw visual values. The timeline reference is authoritative for the All moments timeline band only — rail, Moment cards, and their metrics, subject to ADR-0101's saved-marker and metadata override — not for Home's composition, while the Rediscover row follows the same calm editorial visual language without reproducing the timeline rail or Moment cards.
...
The app palette supplies each chapter's semantic text, surface, border, accent, and control colors in dark mode. The atmospheric background applies the same palette logic as an ordinary Relive canvas: canvas, glow, tint, accent, and shadow are sampled from the two adjacent app palettes and interpolated from the shared scene progress before the broad gradient is drawn. It never swaps a completed brush at a chapter boundary. The carried memory uses `long3` with emphasized easing; grouped supporting elements overlap gently with 8–12dp travel and slight scale. Reduced motion snaps spatial properties and uses only short copy/action fades.

The heading uses the onboarding serif role, body uses the onboarding sans role, primary action uses the onboarding action role, and Skip uses `type.action`. Controls use the active chapter palette's semantic accent/on-accent pair and full-round shape tokens, retain the global `48dp` target, and expose chapter position and action semantics. Decorative atmosphere is excluded from accessibility; adjacent native copy carries its meaning. Large text and compact heights reduce decorative content first and never clip fixed actions.
...
There is no startup-destination row. With a single root there is nothing to choose between: after the one-time first-launch onboarding gate, Relive launches at the top of the Home surface, and Rediscover — a row inside Home rather than a destination — cannot be selected as a start destination. Restoration, incoming-share, and deep-link destinations keep their existing higher priority.
...
## 21A. Media download selection

The full-screen viewer places a 48dp Download icon action opposite Close, using the same mode-independent frosted chrome. Gallery selection begins with long-press; selected tiles retain their media while adding a high-contrast circular check at the top-right. While active, the pinned header hides Back and presents Close at the left, `N selected` centered, and Download at the right. Media uses the same selection language inside its existing rounded sheet. Saving presents a Material 3 determinate progress indicator and Cancel action over blurred, blocked content. The action never changes tile geometry, cover behavior, or the sheet's curved partition. Completion, cancellation, and partial failure use the existing bottom snackbar. Audio and video playback affordances use semantic palette roles rather than fixed light-mode colors.

## 22. Export and portable archive

Export setup is an open Profile canvas with uppercase editorial section labels, full-width controls, generous spacing, and a persistent plain-language privacy disclosure. It is a guided three-part flow: choose the output format first, choose the Timeline and optional date range once for either format, then personalize the cover only when Keepsake PDF is selected. A persistent bottom action area always states the selected Moment count and presents one format-specific creation action. Locked creation actions retain their labels and route to Upgrade. Opening an existing archive is not part of this creation flow: **Open Relive archive** is a separate row in Profile's Your Memories section and is never locked.

Generation and completion are full-screen child states rather than content appended beneath setup. The generation state names the format and current phase, shows determinate progress whenever the platform reports measurable work, retains a visible Cancel action, and consumes system Back by cancelling back to setup. Completion uses a clear success treatment, filename, primary Save to device, secondary Share, and an explicit Create another export action; visible and system Back both return to setup. Errors use the same child-state hierarchy. Transitions use the shared Material 3 forward/back pattern and honor reduced motion.

The Keepsake uses one fixed A4 portrait diary system with six per-export ruled-paper palettes: Warm cream, Blush pink, Sage green, Lavender, Powder blue, and Soft peach. Each palette owns a tested dark ink, stronger ink, accent, rule, and low-contrast doodle treatment; the person chooses paper only and the readable foregrounds change automatically. The selected paper is painted across every physical page before its content, so unused continuation space can never fall back to white. Every sheet is framed by a clearly visible accent-colored rounded double line, with a small centered diamond detail, while an inset content viewport keeps writing and photographs away from the trim and frame. Bundled Kalam carries the expressive hierarchy, with Fraunces/Inter reserved for brand/supporting type, a full cover, and fuller edge-weighted doodle compositions inspired by physical journal marginalia. Its print hierarchy prioritizes comfortable viewing on a phone as well as paper: Moment writing is `18pt`, secondary metadata is `12pt`, compact tags are `10pt`, Moment titles are bold `30pt`, dates are bold `36pt`, and the cover title is bold `48pt`. A felt Moment ends with a standalone `30pt` emoji plus an accessible feeling label; the enum word is not visible copy. Dates, titles, and Moment text are written directly on the page rather than placed inside cards. Every observed date starts with a large ordinal date at the top of a fresh page. A single short Moment rests vertically centered beneath it; multiple Moments from that date flow on the paper and may continue onto additional pages without allowing the next date to share a page. Long writing is broken only at whitespace into balanced page-sized sections before rendering, preventing a handwriting line from being bisected by a physical sheet edge while avoiding a needlessly sparse final continuation; title/time appear once at the start and metadata/tags/feeling appear once at the end. Only the first page names the date; continuation pages are intentionally unlabelled and never say “continued.” Missing dates are never expanded one by one: each uninterrupted gap between observed dates becomes one full-page pause naming the missing span, followed by “No moments were kept here. Life was still being lived.” Text is never truncated. Image attachments appear as lightly rotated, white-bordered photographs with translucent tape and shadow; each frame contains the complete uncropped image, one photo is generous, and two to four photos use count-specific scrapbook compositions. A short image-bearing Moment keeps its first composition beside that Moment on a multi-Moment date; every further composition follows on a complete, fuller A4 photo page before the next Moment begins, so neither narrative ownership nor a photograph crosses a page boundary. Doodles stay deliberately lighter than written content and vary between pages with large organic edge shapes plus botanical sprigs, hearts, stars, paperclips, cameras, suns, rainbows, cups, houses, and curved arrows. Audio and video are omitted completely. Raw coordinates and internal identities are never printed.

The portable archive viewer begins with the exported timeline's frozen cover identity: its name, wallpaper, Moment theme, and custom cover photo. From there it opens that selected date-bounded timeline directly. Moment cards and full-screen media reuse the approved presentation, but every composer, mutation action, Profile destination, theme editor, and floating navigation control is absent.
