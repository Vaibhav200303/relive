...
The automatic bucketed collage cover is used by the `timeline-all` destination card in the external-share timeline picker: its available visual candidates remain a bounded reactive projection, while a deterministic three-hour bucket chooses 1–9 distinct candidates and a curated layout. When there are zero candidates, it uses the neutral no-cover placeholder rather than a generated cover. The `All Photos` card in Home's Rediscover row instead fills with one shuffle-chosen candidate from the same bounded projection (ADR-0064). The `All moments` band on Home carries no cover hero; it begins at its section heading and is not entered from a card.
